...
test("the older bare-id-per-line format still parses", () => {
  assert.deepEqual(parseModelList("gemini-3.8-flash-high\ngemini-3.1-pro-low\n"), [
...
test("decoration around the ids is tolerated", () => {
  const models = parseModelList(
    [
      ...
      "\u001B[32mgemini-3.8-flash-low\u001B[0m\tGemini 3.8 Flash (Low)",
    ].join("\n"),
  );
...
test("prose is not mistaken for a model id", () => {
  ...
  assert.deepEqual(models, ["gemini-3.8-flash-high"]);
});
...
test("a table row does not let a leading status column mask the id", () => {
  ...
  const models = parseModelList("| active-ok | gemini-3.8-flash-high | Gemini 3.8 Flash |\n");
...
test("a display name never contributes fragments of its own", () => {
  // "3.8" is lowercase-ish, starts with a digit and contains a separator — it
  // only stays out because just the first field of a line is considered.
  assert.deepEqual(parseModelList("gemini-3.8-flash-high\tGemini 3.8 Flash (High)"), [
    "gemini-3.8-flash-high",
  ]);
});
