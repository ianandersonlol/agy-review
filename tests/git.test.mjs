...
import {
  ...
  repoRoot,
  resolveBase,
} from "../skills/agy-review/scripts/lib/git.mjs";
...
async function makeRepo() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agy-git-test-"));
...
async function workDir() {
  return fs.mkdtemp(path.join(os.tmpdir(), "agy-git-work-"));
}
...
test("repoRoot returns null outside a repository", { skip }, async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agy-not-a-repo-"));
  try {
...
test("an untracked symlink is never dereferenced into the diff", { skip: skip || process.platform === "win32" ? "needs POSIX symlinks" : false }, async () => {
  ...
  const { dir } = await makeRepo();
  const work = await workDir();
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "agy-outside-"));
  try {
